...
Система использует упрощённую модель доступа с двумя типами аккаунтов. У обеих ролей один уровень разрешений: если у роли есть
доступ к сущности, то она может просматривать и изменять данные в рамках закреплённых за ней клиентов и сделок; дополнительных уровней
«только чтение» или «расширенный доступ» нет.

- **Администратор** — управляет пользователями и системными настройками, может просматривать и изменять любые сущности для аудита и поддержки.
- **Пользователь** — универсальная роль для сотрудников бизнеса: такие пользователи создают и ведут клиентов, сделки, полисы, платежи, документы и задачи, если назначены ответственными за соответствующий объект.
...
Инициация сделки. Назначенный ведущий пользователь создаёт клиента (если ещё не существует), заводит новую сделку, записывает в журнал ключевые параметры (например, пробег автомобиля, остаток долга). Создаёт папку сделки и складывает первичные документы. Назначает коллегам задачи на подготовку расчётов.
