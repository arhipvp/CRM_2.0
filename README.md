...
2. предзапускную проверку переменных `./scripts/check-env.sh --env-file .env`, которая выявляет пропущенные значения, пустые секции и плейсхолдеры `change_me` до старта Docker Compose;
3. запуск инфраструктуры `docker compose --env-file .env up -d` в каталоге `infra/`;
   ...
   ```
4. ожидание готовности инфраструктурных контейнеров (`docker compose wait` либо резервный цикл с проверкой healthcheck);
5. автоматическую настройку RabbitMQ (`infra/rabbitmq/bootstrap.sh`) — обмены и очереди появляются до старта прикладных сервисов;
6. миграции CRM/Auth через `scripts/migrate-local.sh`, пока инфраструктура готова, а backend-профиль ещё не поднят;
   > ⚠️ При любом ручном запуске CRM API обязательно выполните `poetry run alembic upgrade head` в каталоге `backend/crm`.
   > Приложение проверяет ревизию на старте и завершится с ошибкой, если база отстаёт от `2025102604_migrate_crm_tasks_to_tasks_schema`.
7. smoke-проверку `BACKUP_*`: при пустых `BACKUP_S3_*` скрипт сообщает о переходе на `DummyStorage` (штатный режим локальной разработки) и продолжает выполнение, а при частично заполненной конфигурации S3 — останавливается с подсказкой;
8. дополнительную smoke-проверку, которая удостоверяется, что контейнер `backup` действительно запущен в режиме `DummyStorage`, когда `BACKUP_S3_*` пусты;
9. старт профильных API `docker compose --env-file .env --profile backend up -d` (Gateway, Auth, CRM, Documents, Frontend; поддерживается флаг `--skip-backend` или переменная `BOOTSTRAP_SKIP_BACKEND=true` для ограничения только инфраструктурой);
10. ожидание готовности backend-профиля через helper `check_backend_services`, который циклически анализирует `docker compose --profile backend ps --format json` (при недоступности JSON вывод разбирается из табличного `docker compose ps`; поддержка `--format json` появилась в Compose V2, поэтому обновление до актуальной версии остаётся рекомендованным); при необходимости можно передать `--skip-backend-wait` или `BOOTSTRAP_SKIP_BACKEND_WAIT=true`, чтобы пропустить ожидание и перейти к следующим шагам;
11. опциональный запуск локальных backend-процессов через `scripts/start-backend.sh` (активируется комбинацией `--with-backend --skip-backend` или переменными `BOOTSTRAP_WITH_BACKEND=true` и `BOOTSTRAP_SKIP_BACKEND=true`, PID, журналы сервисов и файл запуска по умолчанию сохраняются в `.local/run/backend`; путь можно переопределить опцией `--log-file` или переменной `START_BACKEND_LOG_FILE`). Для запуска только части сервисов используйте аргумент `--service NAME` у `scripts/start-backend.sh` (можно перечислять через запятую и повторять флаг, например `scripts/start-backend.sh --service gateway` или `scripts/start-backend.sh --service auth,crm-api --service gateway`). Например, чтобы ограничиться Gateway и CRM API, выполните `scripts/start-backend.sh --service gateway,crm-api`.
12. загрузку seed-данных, если существует `scripts/load-seeds.sh` (скрипт работает только с SQL-файлами Postgres и больше не запускает легаси Tasks);
13. smoke-проверку окружения `scripts/check-local-infra.sh` (PostgreSQL, Redis, Consul, RabbitMQ UI, Reports) и REST/SSE эндпоинтов backend-профиля (Gateway, Auth, CRM, Documents, Frontend UI).

### Проверка `.env`

`./scripts/check-env.sh` сверяет текущие `.env` с шаблоном `env.example`, раскрывает ссылки вида `${VAR}` и сообщает об обязательных переменных, которые остались пустыми или содержат плейсхолдеры (`change_me`, `ChangeMePlease123!`, dev-токены Telegram и т.п.). Скрипт используется внутри `bootstrap-local.sh`, поэтому пайплайн останавливается до запуска Docker Compose, если переменные заполнены некорректно. Выполняйте его вручную перед `docker compose`, чтобы быстрее находить пропуски при ручном редактировании `.env`:

```bash
./scripts/check-env.sh            # корневой .env
./scripts/check-env.sh --all      # корневой .env и backend/{gateway,auth,crm,documents,reports}/.env
```

При необходимости можно указать конкретный файл: `./scripts/check-env.sh --env-file backend/gateway/.env`. Все проверки выводятся в табличном отчёте — ошибки завершают скрипт с ненулевым кодом возврата, а предупреждения помогают заменить плейсхолдеры на реальные секреты.
...
В репозитории уже лежат рабочие backend-сервисы первой волны: **Gateway/BFF** (NestJS + pnpm), **Auth** (Spring Boot + Gradle) и **CRM/Deals** (FastAPI + Celery) со встроенными модулями задач, уведомлений и платежей, которые предоставляют REST/SSE API и публикуют события от имени CRM.【F:docs/api/tasks.md†L1-L39】【F:docs/api/notifications.md†L1-L33】 Каркас сервиса **Documents** и инфраструктурного набора **Backup** подготовлен вместе с миграциями и инструкциями; прикладная логика развивается согласно `docs/delivery-plan.md`. Каталог **Reports** содержит рабочий каркас FastAPI-сервиса с подключением к PostgreSQL (см. [`backend/reports/README.md`](backend/reports/README.md)). Центрального сервиса аудита нет: прикладные системы ведут собственные журналы, а витрины Reports агрегируют события для ретроспективного анализа. Gateway обслуживает внутренних клиентов (CRM UI через BFF), Telegram-бота и внешние интеграции, поэтому первичные точки взаимодействия описаны в `docs/architecture.md` и `docs/api`. Детали по окружению и API остаются в `docs/tech-stack.md`, `docs/api` и `env.example`. Легаси-код задач удалён: единственная поддерживаемая реализация входит в [`backend/crm`](backend/crm/README.md) и описана в [`docs/api/tasks.md`](docs/api/tasks.md); каталог [`backend/notifications`](backend/notifications/README.md) сохранён как справочный архив. Активный платежный модуль поставляется внутри CRM, а каталог [`backend/payments`](backend/payments) содержит только архивные материалы standalone-сервиса.
...
- [`backend/payments/README.md`](backend/payments/README.md) — архивная документация standalone-сервиса Payments; актуальный платёжный модуль поставляется внутри [`backend/crm`](backend/crm/README.md) и именно CRM публикует API и события.
  ...
  - Модуль задач больше не поставляется отдельным сервисом: все REST/SSE API реализованы внутри CRM (`backend/crm`, `docs/api/tasks.md`). Каталог [`backend/notifications`](backend/notifications/README.md) оставлен как справочный архив; действующие уведомления описаны в CRM (`docs/api/notifications.md`).【F:docs/api/tasks.md†L1-L39】【F:docs/api/notifications.md†L1-L33】
...
Documents. Связывает сделки и полисы с каталогами серверного хранилища, хранит метаданные файлов, управляет доступами. Сервис **не** вызывает `setfacl` автоматически — расширенные ACL обновляются вручную или внешними скриптами администраторов. Генерации документов нет; подготовленные инструкции и примеры команд `setfacl` приведены в [`backend/documents/README.md`](backend/documents/README.md#права-доступа-и-пользователи).
...
- модуль аутентификации, который после внедрения привязки Telegram-аккаунтов в Auth будет проверять связь пользователя с ботом.
