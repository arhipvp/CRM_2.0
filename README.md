...
8. старт профильных API `docker compose --env-file .env --profile backend up -d` (Gateway, Auth, CRM, Documents, Frontend; поддерживается флаг `--skip-backend` или переменная `BOOTSTRAP_SKIP_BACKEND=true` для ограничения только инфраструктурой);
9. ожидание готовности backend-профиля через helper `check_backend_services`, который циклически анализирует `docker compose --profile backend ps --format json` (при недоступности JSON вывод разбирается из табличного `docker compose ps`; поддержка `--format json` появилась в Compose V2, поэтому обновление до актуальной версии остаётся рекомендованным); при необходимости можно передать `--skip-backend-wait` или `BOOTSTRAP_SKIP_BACKEND_WAIT=true`, чтобы пропустить ожидание и перейти к следующим шагам;
...
12. smoke-проверку окружения `scripts/check-local-infra.sh` (PostgreSQL, Redis, Consul, RabbitMQ UI, Reports) и REST/SSE эндпоинтов backend-профиля (Gateway, Auth, CRM, Documents, Frontend UI).
