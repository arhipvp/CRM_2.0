...
Gateway/BFF. Общая точка входа для фронтенда, управление сессиями, маршрутизация запросов к сервисам, агрегация данных (подробнее см. раздел [«Фронтенд» технологического стека](docs/tech-stack.md#фронтенд)). Фронтенд использует REST API шлюза, SSE-канал для изменений сделок и WebSocket-подключение для уведомлений.
