...
**Быстрая навигация:** [План поставки](docs/delivery-plan.md), [Архитектура](docs/architecture.md#1-общая-структура-сервисов), [Технологический стек](docs/tech-stack.md), [API](docs/api/README.md), [UX-документация фронтенда](docs/frontend/README.md), [Локальная настройка](docs/local-setup.md), [Тестовые данные](docs/testing-data.md)
...
Сервисы первой поставки уже заведены с рабочим кодом и тестами: **Gateway/BFF** (NestJS + pnpm), **Auth** (Spring Boot + Gradle), **CRM/Deals** (FastAPI + Celery) и **Frontend** (Next.js + pnpm). Для сервисов **Payments**, **Documents**, **Tasks**, **Notifications**, **Reports**, **Audit** и **Backup** подготовлены каталоги, миграции и инструкции, но их бизнес-логика ещё не реализована — ориентируйтесь на `docs/delivery-plan.md` и README соответствующих папок при планировании следующих инкрементов. Инфраструктурные описания, требования к окружению и согласованные API остаются в `docs/tech-stack.md`, `docs/api` и `env.example`.
...
Tasks. Планирование и исполнение задач, напоминания назначенным пользователям. SLA и автоматические сценарии будут включены после выпуска [Этапа 1.1](docs/delivery-plan.md#2-приоритизация-последующих-этапов). Подробнее см. раздел [«Tasks» технологического стека](docs/tech-stack.md#tasks).
