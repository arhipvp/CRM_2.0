...
Если требуется один сценарий с дополнительными опциями, используйте `./scripts/dev-up.sh` — он остаётся обёрткой вокруг bootstrap-скрипта, переиспользует те же шаги и добавляет централизованное логирование. Набор ключевых флагов `dev-up`:

- `--skip-backend` — передаёт в bootstrap решение полностью пропустить compose-профиль backend, чтобы ограничиться инфраструктурой;
- `--skip-backend-wait` — отменяет ожидание готовности профиля backend (аналог переменной `DEV_UP_SKIP_BACKEND_WAIT=true`) и позволяет перейти к ручной проверке healthcheck;
- `--with-backend` — активирует запуск `scripts/start-backend.sh` (или `DEV_UP_WITH_BACKEND=true`) и выводит предупреждение при одновременном запуске compose-профиля backend;
- `--log-file PATH` — сохраняет журнал в указанный путь (аналог `DEV_UP_LOG_FILE`), по умолчанию `.local/logs/dev-up.log` дополняется при каждом запуске.

При запуске `./scripts/dev-up.sh --skip-backend` соответствующий профиль автоматически пропускается в bootstrap, поэтому контейнеры не стартуют ни на одном этапе. Журнал работы `dev-up` пишется через `tee` (поведение можно настроить флагом `--log-file` или переменной `DEV_UP_LOG_FILE`; относительные пути интерпретируются относительно корня репозитория). Автоматическое открытие браузера в сценарии не реализовано — при необходимости откройте интерфейс вручную. Ручные вызовы `scripts/start-backend.sh` (включая `--service`) и `scripts/stop-backend.sh` остаются совместимыми с автоматическими сценариями — bootstrap и `dev-up` используют те же helpers и корректно обрабатывают уже запущенные процессы.
