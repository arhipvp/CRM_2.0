...
- Backend:
  ...
  - [`backend/payments/README.md`](backend/payments/README.md) — архивная документация прежнего сервиса Payments; модуль больше не используется.
...
Атрибуты оплаты фиксируются одной записью: сумма, валюта, фактическая дата (`actual_date`), комментарий и пользователь, подтвердивший оплату. Плановых дат, статусов и отдельных записей на исходящие выплаты больше нет — при корректировках обновляется та же запись. Интеграции с банками/бухгалтерией не требуются: ввод ручной или через импорт файлов пользователей.
...
Платежи (модуль CRM). Учёт входящих оплат по полисам, подтверждение фактической даты, комментарии продавца и аудит изменений. Отдельный сервис Payments больше не используется — платежи обслуживаются внутри CRM/Deals.
...
Комиссия и расходы. Ответственный за сделку пользователь инициирует записи о комиссионных, скидках и выплатах, а коллега, ведущий финансовый контроль, подтверждает суммы и статусы. Учёт оплат ведётся в CRM/Deals: отдельный сервис Payments больше не используется.
