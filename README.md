...
Payments. Учёт всех типов платежей, план-факт анализ, контроль комиссий и выплат. Реализация в стадии каркаса, параметры окружения и план миграций собраны в [`backend/payments/README.md`](backend/payments/README.md). API сервиса принимает тела запросов как в `camelCase`, так и в `snake_case`, чтобы обеспечить совместимость с клиентами разных код-стилей.

Documents. Связывает сделки и полисы с папками Google Drive, хранит метаданные файлов, управляет доступами. Генерации документов нет; подготовленные инструкции — в [`backend/documents/README.md`](backend/documents/README.md). API `GET /documents` возвращает массив записей, а общее количество документов передаётся через заголовок `X-Total-Count` для пагинации.
