...
Администратор — управляет пользователями, ролями и справочниками страховых компаний/продуктов.

Продавец (агент) — инициирует сделки, общается с клиентом, получает комиссию, ведёт журнал сделки и фиксирует расходы/выплаты.

Исполнитель — подготавливает расчёты, оформляет полисы, поддерживает документооборот и контролирует продления.

Финансовый менеджер — сверяет и подтверждает платежи, контролирует комиссионные и выплаты исполнителям.

Руководитель — отслеживает метрики, воронку продаж и эффективность команды.
...
Платёж. Любое движение денег, связанное со сделкой/полисом:
...
Атрибуты: сумма, валюта, даты плановая и фактическая, статус (запланирован, ожидается, получен, выплачен), связанный полис и сделка. Интеграции с банками/бухгалтерией не требуются: ввод ручной или через импорт файлов пользователей.
...
Notifications. Отправка уведомлений по событиям (email, мессенджеры, push).

Reports. Построение аналитики по продажам, продлениям, доходности и эффективности.
...
Documents-сервис создаёт и поддерживает структуру папок, выдаёт ссылку, проверяет доступы.
...
Оформление и оплата. Клиент оплачивает полис. Исполнитель отражает факт оплаты в разделе платежей, после чего вносит оформленный полис в карточку сделки и добавляет сканы в соответствующую папку.

Комиссия и расходы. Продавец фиксирует получение комиссионного вознаграждения, регистрирует скидки клиенту (если есть) и выплаты исполнителю. Сервис Payments отслеживает состояние всех связанных платежей.
