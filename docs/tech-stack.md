...
Сервис работает с файловой системой через стандартные вызовы `fs.mkdir`, операции копирования и удаления, не назначая POSIX-права и ACL самостоятельно. Он полагается на уже настроенные разрешения целевого каталога и пользователя, под которым запущен процесс. Настройка прав доступа и ACL выполняется администратором вручную по инструкции из [`backend/documents/README.md`](../backend/documents/README.md#права-доступа-и-пользователи), чтобы каталог соответствовал ожидаемой модели безопасности.
...
Служебные файлы после загрузки обрабатываются воркерами BullMQ: задача `documents.upload` переносит данные из временного объекта в каталог `DOCUMENTS_STORAGE_ROOT` и фиксирует итоговые параметры, а повторная синхронизация (`documents.sync`) перечитывает файловую систему, обновляет хэши/размеры и восстанавливает статус документа после ошибок. Эти задачи выполняются только при запущенном воркере и не включают политику автоматического архивирования.

Автоматическое удаление файлов пока не реализовано: сервис не формирует задания `documents.cleanup` и не инициирует перенос устаревших документов. Для соблюдения политик хранения требуется внешний процесс (cron-задача, сторонний воркер или оператор хранилища), который будет вызывать API удаления и синхронизации. План внедрения штатного механизма очистки ещё не утверждён.
...
RabbitMQ выступает шиной доменных событий: сервис публикует изменения в exchange `crm.events` (`deal.journal.appended`, `deal.calculation.created|updated|deleted|status.*`, семейство `deal.payment.*`, события документов; SLA-события добавятся после Этапа 1.1). Модуль задач выделен в отдельный exchange `tasks.events` с routing key `task.created`, `task.status.changed`, `task.reminder`, которые читают уведомления и фронтовые каналы; CRM не ретранслирует их в `crm.events`. Встроенные модули задач и уведомлений подписываются на соответствующие routing key (см. [архитектуру, раздел 2.2](architecture.md#22-асинхронная-шина-rabbitmq)) и ретранслируют события в SSE/Telegram. Подключение реализовано поверх `aio-pika` с подтверждением доставки (publisher confirms) и ручным ack со стороны консьюмера. Отдельных подписок на `payments.events` больше нет — платежи ведутся внутри CRM, а события по доходам/расходам и агрегатам платежа закрывают все обновления без обобщённого `deal.updated`.
...
Для Auth в дорожной карте (Этап 1.1) остаётся привязка Telegram-аккаунтов и подтверждение операций через бота; требования уточняются в [плане поставки](delivery-plan.md) и пока не поддерживаются текущим REST API.【F:docs/delivery-plan.md†L1-L35】
