...
Материал дополняет обзорные документы (README.md, architecture.md, security-and-access.md) и задаёт общие правила для детальных спецификаций отдельных сервисов. За обзором взаимодействий и потоков данных обращайтесь к разделу [«Взаимодействия и потоки данных»](architecture.md#2-взаимодействия-и-потоки-данных).
...
Интеграция с внешним провайдером валютных курсов
-------------------------------------------------

* Провайдер: Open Exchange Rates — https://openexchangerates.org/ (REST API `https://openexchangerates.org/api`).
* Частота обновления: официальные данные обновляются провайдером раз в час; сервис Payments обновляет кеш через Gateway каждые 5 минут, чтобы подхватывать последние значения.
* Требования к доступу: обязательный параметр `app_id`, передаваемый как API-ключ (переменная `PAYMENTS_CURRENCY_API_APP_ID`, хранится в Secrets Manager и подставляется в Gateway).

Gateway маршрутизирует запросы Payments в Open Exchange Rates и инкапсулирует сетевую политику:

* Публичный маршрут Gateway: `GET /api/v1/payments/rates/latest`.
* Прокси-эндпоинт к провайдеру: `GET {PAYMENTS_CURRENCY_API_URL}/latest.json?base={base}&app_id=***`.
* Формат запроса: HTTP GET, заголовки `Accept: application/json`, дополнительный параметр `symbols` (необязательный) с перечислением валют.
* Формат ответа: JSON `{ "timestamp": 1700000000, "base": "USD", "rates": { "EUR": 0.93, "RUB": 92.1, ... } }`; Gateway транслирует структуру без изменений и добавляет служебные заголовки кэширования.

Стратегия устойчивости и отказов:

* Gateway выполняет до трёх повторных попыток с экспоненциальной задержкой (по 200/400/800 мс) и отвечает за кэширование результата в Redis (`payments:fx:latest:{base}`) на 5 минут.
* При недоступности провайдера возвращается последний валидный ответ из кеша с флагом `X-Rate-Stale: true`; если данных нет или они старше 24 часов, Payments переводит операции, требующие пересчёта, в состояние «ожидание курсов» и отправляет алерт в Notifications.
* Gateway ведёт метрики доступности (`gateway.payments.fx.*`) и повышает уровень алерта, если подряд наблюдаются 5 неудачных обновлений.
