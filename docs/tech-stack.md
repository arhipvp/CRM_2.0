...
RabbitMQ выступает шиной доменных событий: сервис публикует изменения в exchange `crm.events` (`deal.created`, `deal.updated`, `deal.payment.updated`, документы; SLA-события добавятся после Этапа 1.1). Notifications и Tasks подписываются на соответствующие routing key (см. [архитектуру, раздел 2.2](architecture.md#22-асинхронная-шина-rabbitmq)). Подключение реализовано поверх `aio-pika` с подтверждением доставки (publisher confirms) и ручным ack со стороны консьюмера. Отдельных подписок на `payments.events` больше нет — платежи ведутся внутри CRM.
