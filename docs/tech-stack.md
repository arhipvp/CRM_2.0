...
RabbitMQ выступает шиной доменных событий: сервис публикует изменения в exchange `crm.events` (`deal.created`, `deal.updated`, `deal.payment.updated`, `deal.payment.cashflow.synced`, документы; SLA-события добавятся после Этапа 1.1). Notifications и Tasks подписываются на соответствующие routing key (см. [архитектуру, раздел 2.2](architecture.md#22-асинхронная-шина-rabbitmq)). Подключение реализовано поверх `aio-pika` с подтверждением доставки (publisher confirms) и ручным ack со стороны консьюмера. Отдельных подписок на `payments.events` больше нет — платежи ведутся внутри CRM, а отдельное событие cashflow позволяет отчётам получать агрегаты доходов/расходов без опроса основного API.
...
Платёжная логика встроена в CRM/Deals: модуль платежей CRM заменил прежний сервис Payments. CRM ведёт множество записей на каждый полис, позволяя фиксировать частичные поступления и распределять комиссию. Каждая запись (`crm.policy_payments`) хранит направление (`incoming`/`outgoing`), назначение, сумму и фактическую дату, а детализированные позиции (`crm.policy_payment_incomes`/`crm.policy_payment_expenses`) описывают структуру доходов и расходов. Модуль публикует события `deal.payment.updated` (обновление базовой записи) и `deal.payment.cashflow.synced` (смена состава позиций) в `crm.events`, а Audit фиксирует все корректировки. Архивная документация прежнего сервиса сохранена в каталоге `backend/payments` для справки и миграционных сценариев.
Платёжная логика встроена в CRM/Deals: модуль платежей CRM заменил прежний сервис Payments. CRM хранит несколько записей платежей по полису с детализированными позициями доходов и расходов, автоматически агрегирует суммы, публикует событие `deal.payment.updated` в `crm.events` и фиксирует правки в Audit. Архивная документация прежнего сервиса сохранена в каталоге `backend/payments` для справки и миграционных сценариев.
