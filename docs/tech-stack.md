...
Documents-сервис больше не использует Drive API: файлы и папки размещаются в серверном каталоге, смонтированном в контейнер/виртуальную машину (локальный путь, подключённый том или S3-совместимый бакет, примонтированный через s3fs/rgw). Корневой каталог задаётся переменной `DOCUMENTS_STORAGE_ROOT`, шаблоны вложенных структур остаются прежними и строятся относительно этого пути.
