...
БД и очереди: Spring Data R2DBC (PostgreSQL), Spring AMQP (RabbitMQ — exchange `payments.events`, очереди `payments.exports`/`payments.exports.status`)
...
Очереди RabbitMQ payments.exports / payments.exports.status для асинхронных выгрузок платежей (воркер получает фильтры и настройки хранилища из `PAYMENTS_EXPORTS_*`)
...
* Экспорт платежей выполняется асинхронно: сервис кладёт задание в очередь `PAYMENTS_EXPORTS_QUEUE`, обновления статусов (`processing` → `done`/`failed`) приходят из `PAYMENTS_EXPORTS_STATUS_QUEUE`, ссылки на скачивание формируются воркером с учётом TTL `PAYMENTS_EXPORTS_STORAGE_URL_TTL`.
...
JUnit5 + Testcontainers для PostgreSQL и RabbitMQ (включая проверку очередей экспорта)
