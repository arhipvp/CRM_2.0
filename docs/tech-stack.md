...
GitHub Actions выполняет многоступенчатый конвейер `ci.yml`: матричный запуск lint/unit/contract-тестов для Gateway (Node.js/pnpm) с заготовками под Gradle (Auth) и Poetry (CRM) сервисы, а также сборку и публикацию OCI-образов в GHCR с кэшированием Docker-слоёв (`cache-from/cache-to` на BuildKit).
