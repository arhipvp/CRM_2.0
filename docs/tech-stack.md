...
Платёжная логика встроена в CRM/Deals: модуль платежей CRM заменил прежний сервис Payments. На каждый полис можно завести несколько платежей, каждый из которых хранит плановые параметры, статус и агрегированные суммы операций поступлений/удержаний. Фактические движения фиксируются в таблице `crm.payment_transactions`, а события `deal.payment.created/updated/deleted` и `deal.payment.transaction.*` публикуются в `crm.events`. Архивная документация прежнего сервиса сохранена в каталоге `backend/payments` для справки и миграционных сценариев.
