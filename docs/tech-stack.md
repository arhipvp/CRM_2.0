...
API: REST (JSON) для фронтенда + SSE-каналы для событий задач и платежей, внутренние вызовы — REST/gRPC
Трансляция событий: Gateway подписывается на RabbitMQ (`tasks.events` и подписки на `payments.events.*`), агрегирует сообщения о смене статусов задач и платежей и отдаёт их фронтенду через SSE-потоки, доступные в публичном `/api`-контуре.
