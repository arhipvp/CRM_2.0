...
Взаимодействие с Gateway/BFF: REST-запросы через обёртку вокруг `fetch` с автоматическим проставлением токена сессии (httpOnly cookie); подключены SSE-каналы для уведомлений задач и платежей. Все вызовы проходят через `/api` шлюз, домен которого задаётся переменными окружения.
