...
Внутренние справочники тарифов и валютных коэффициентов, задаваемые через административные инструменты CRM

Работа со справочниками
-----------------------

* Базовая валюта CRM фиксирована в конфигурации сервиса; пересчёты выполняются по таблицам коэффициентов, которые поддерживаются операторами финансового блока.
* Значения коэффициентов версионируются миграциями и административными действиями; изменения фиксируются в Audit вместе с автором и датой публикации.
* Gateway предоставляет REST-эндпоинты Payments только для внутренних клиентов (`/api/v1/payments/**`) и не использует внешние маршруты для конвертации.
