...
Платёжная логика встроена в CRM/Deals: отдельного сервиса Payments больше нет. CRM хранит единственную запись оплаты по каждому полису, обновляет фактическую дату и комментарий продавца, публикует событие `deal.payment.updated` в `crm.events` и фиксирует правки в Audit. Архивная документация прежнего сервиса сохранена в каталоге `backend/payments` для справки и миграционных сценариев.
