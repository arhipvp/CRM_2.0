...
5. **Блокировка или увольнение**
   - Главный админ отключает пользователя в Auth через установку `user.enabled = false`, что мгновенно:
     - Блокирует новые попытки логина (ответ 403 Forbidden)
     - Инвалидирует все существующие refresh-токены
     - Прекращает доступ при попытке обновления сессии
