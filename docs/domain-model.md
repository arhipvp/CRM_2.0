...
| amount | Decimal | Да | Совокупная сумма платежа с учётом всех привязанных позиций доходов и расходов. |
| currency | String | Да | Валюта, фиксированное значение `RUB` (для обратной совместимости со старыми выгрузками). |
| actual_date | Date | Да | Фактическая дата последнего движения по платежу; обязательна и определяет статус оплаты полиса. |
| recorded_by_id | UUID | Да | Пользователь (продавец/аккаунт-менеджер), который зафиксировал или обновил платёж. |
...
**Статья дохода/расхода платежа**

| Поле | Тип | Обязательное | Описание |
| --- | --- | --- | --- |
| id | UUID | Да | Уникальный идентификатор позиции. |
| payment_id | UUID | Да | Ссылка на платеж. |
| type | Enum (`income`, `expense`) | Да | Признак направления движения средств. |
| category | String | Да | Тип статьи (например, «Клиентская оплата», «Комиссия агента», «Возврат премии»). |
| amount | Decimal | Да | Сумма по позиции, всегда положительная. |
| occurred_on | Date | Да | Дата фактического начисления/списания. |
| recorded_by_id | UUID | Да | Пользователь, подтвердивший позицию. |
| reference | String | Нет | Связанный внешний идентификатор (платёжное поручение, счёт, чек). |
| notes | Text | Нет | Комментарий с деталями расчёта. |

> **Примечания:**
> - Общая сумма платежа (`amount`) вычисляется как разница между суммами доходов и расходов и хранится денормализованно для быстрого доступа.
...
- Итоговая сумма платежа пересчитывается из списка позиций (`доходы` минус `расходы`) и хранится денормализованно для ускорения отчётности.
