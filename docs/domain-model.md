...
| next_review_at | DateTime | Нет | Дата и время ближайшего обязательного обзора. Используется для сортировки карточек и напоминаний. |
...
| --- | --- | --- | --- |
| `planned` | Запланирован | Платёж внесён в график, счёт ещё не выставлен. | Добавление строки графика платежей, импорт планов из таблицы. |
| `expected` | Ожидается | Счёт отправлен, ожидается подтверждение поступления или списания. | Пользователь, ведущий сделку, фиксирует отправку счёта либо подтверждение от клиента. |
| `received` | Получен | Поступление средств на счёт подтверждено. | Назначенный для финансового контроля пользователь вручную фиксирует подтверждение платежа в системе (ручной ввод/подтверждение пользователем). |
...
| `cancelled` | Отменён | Платёж отменён либо перенесён без движения денег. | Любой пользователь с правами редактирования платежей фиксирует отмену либо перенос, указав причину в `notes`. |
