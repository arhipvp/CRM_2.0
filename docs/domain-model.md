...
| currency | String | Да | Валюта, фиксированное значение `RUB`; колонка служит для обратной совместимости с историческими интеграциями и не поддерживает другие значения. |
...
| Код | Локализованный статус | Описание | Как переводится в статус |
| --- | --- | --- | --- |
| `planned` | Запланирован | Платёж добавлен в график, но счёт ещё не выставлен. | Добавление строки графика платежей, импорт планов из таблицы. |
| `expected` | Ожидается | Счёт выставлен, ожидается подтверждение поступления/списания. | Пользователь, ведущий сделку, фиксирует отправку счёта либо подтверждение от клиента. |
| `received` | Получен | Денежные средства поступили на счёт компании. | Назначенный для финансового контроля пользователь вручную фиксирует подтверждение платежа в системе (ручной ввод/подтверждение пользователем). |
| `paid_out` | Выплачен | Сумма перечислена контрагенту или распределена как комиссия/скидка. | Финансово ответственный пользователь подтверждает исходящий платёж или распределение комиссии без дробления суммы. |
| `cancelled` | Отменён | Платёж отменён или перенесён: обязательство закрыто без движения денег. | Любой пользователь с правами редактирования платежей фиксирует отмену либо перенос, указав причину в `notes`. |

Удаление платежей не допускается: любые корректировки выполняются через редактирование существующей записи с обязательным сохранением истории изменений; новые записи при исправлениях не создаются. Ошибочно созданные или не состоявшиеся платежи переводятся в соответствующий статус (`planned`, `expected` или `cancelled`) с обязательным пояснением в `notes`; изменение сумм, дат и реквизитов выполняется в рамках той же записи с автоматической фиксацией версии в `audit_log`.

**Переходы:**
- `planned` → `expected` — после отправки счёта/заявки на оплату.
- `expected` → `received` — при подтверждении входящего платежа.
- `received` → `paid_out` — при отражении исходящего платежа полной суммой или подтверждении распределения комиссии без дробления.
- `expected` ↔ `planned` — допускается возврат к предыдущему статусу при корректировке параметров платежа.
- `received` ↔ `expected` — корректировка реквизитов или суммы выполняется через редактирование существующей записи с обязательной фиксацией истории изменений.
- `paid_out` ↔ `received` — изменение получателя полной суммы оформляется редактированием записи; история версий сохраняется автоматически.
- Любой статус → `cancelled` — при подтверждённой отмене или переносе платежа; требуется комментарий с причиной.

## Задачи

| Статус | Описание | Как переводится в статус |
| --- | --- | --- |
| Новая (`new`) | Создана карточка задачи с назначенным исполнителем и сроком. | Автоматически при создании пользователем или системным процессом, связанным с событиями из раздела процессов. |
