...
- Любой статус → Запланирован — допустимо только при создании новой записи; обратные переходы не используются, корректировки проводятся через отмену и создание новой записи.
- Получен → Запланирован/Ожидается — при ошибке оператор оформляет сторно и создаёт новую запись (исторически фиксируется). 
