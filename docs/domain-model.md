...
| created_by_id | UUID | Да | Пользователь, создавший запись платежа. |
| updated_by_id | UUID | Нет | Пользователь, последним изменивший запись платежа. |
...
> **Примечание:** операции доходов и расходов хранятся в отдельных таблицах (`crm.payment_incomes` и `crm.payment_expenses`); направление определяется самим набором данных и используемыми endpoint'ами `/incomes` и `/expenses`.
