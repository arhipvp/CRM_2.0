...
Платёжный контур поддерживает несколько записей на один полис. Каждая запись описывает плановые параметры (сумма, дата, комментарий), статус (`scheduled`, `partially_paid`, `paid`, `cancelled`) и агрегирует связанные операции поступлений (`incomes`) и удержаний (`expenses`).

- Пользователь может заводить любое количество платежей: авансы, финальные выплаты, корректировки. Номер `sequence` определяет порядок отображения на таймлайне полиса.
- Плановая информация (`planned_amount`, `planned_date`) редактируется до тех пор, пока по платежу нет операций. После появления поступлений/расходов изменение суммы или валюты допускается только пользователями с ролью `financial_manager`.
- Фактическое закрытие фиксируется либо вводом `actual_date`, либо достижением статуса `paid`, когда сумма поступлений минус удержания (`net_total`) соответствует плановой сумме. При ручном закрытии CRM сохраняет пользователя (`recorded_by_id`), который подтвердил факт поступления.
- Каждая операция поступления/расхода содержит категорию (`wire`, `cash`, `agency_fee`, `refund` и т.д.), дату `posted_at`, сумму и автора. Операции можно редактировать и удалять; история правок записывается в аудит.
- Суммы удержаний не списываются автоматически из бухгалтерии: CRM фиксирует только факт движения средств по сделке и публикует события `deal.payment.*` для синхронизации с отчётами.
