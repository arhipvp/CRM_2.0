...
| tenant_id | UUID | Да | Арендатор, которому принадлежит запись клиента. |
| owner_id | UUID | Да | Ответственный пользователь CRM, управляющий карточкой. |
...
| tenant_id | UUID | Да | Арендатор, которому принадлежит запись. |
| client_id | UUID | Да | Ссылка на клиента. |
| owner_id | UUID | Да | Ответственный пользователь сделки. |
...
| value | Decimal | Нет | Денежная оценка сделки. |
...
| status | Enum | Да | Текущий статус документа (`pending_upload`, `ready`, `failed`, `deleted`). |
...
| Подтверждён | Клиент выбрал вариант, расчёт привязан к согласованному полису и заблокирован от редактирования. | Продавец или руководитель сделки подтверждает расчёт и при необходимости указывает связанный полис. |
...
- Готов → Подтверждён — продавец фиксирует выбор клиента и при необходимости привязывает полис.
