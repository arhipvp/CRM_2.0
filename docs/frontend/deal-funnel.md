...
- Ответы CRM приходят в snake_case (`title`, `client_id`, `next_review_at`, `owner_id`). `ApiClient` нормализует их до интерфейса `Deal`, подгружая подписи клиентов через `/crm/clients`. Если имя клиента пустое, в карточке и таблице отображается его идентификатор. Параметр `probability` отсутствует в API, поэтому пока выводится значение `0%`.
...
- При `NEXT_PUBLIC_API_BASE_URL ≠ mock` клиент возвращает ошибки CRM напрямую (таймауты, 4xx/5xx). Автономный режим с возвратом мок-данных активируется только при явном значении `NEXT_PUBLIC_API_BASE_URL=mock`.
