...
- **Главная страница.** Виджет обёрнут в `HomeDealFunnelBoard`, который при монтировании сохраняет фильтры, массовый выбор, предпросмотр и подсветки из `/deals`, затем сбрасывает состояние до `createDefaultDealFilters()`, очищает `selectedDealIds`, закрывает предпросмотр и отключает подсветку, чтобы на `/` всегда отображалась дефолтная канбан-доска (`forceViewMode="kanban"`). При размонтировании компонент возвращает сохранённые фильтры и выбор, поэтому изменения фильтров и выделения, сделанные на главной, не переносятся обратно в основную воронку. Новый предпросмотр или подсветки, появившиеся во время работы виджета (в том числе через SSE), остаются активными после перехода на `/deals`.
...
- **Главная ↔ страница сделок.** `HomeDealFunnelBoard` делает снимок состояния `/deals`, сбрасывает его до значений по умолчанию для домашнего виджета и при размонтировании возвращает сохранённые фильтры и выбранные сделки. Поэтому изменения фильтров и массового выбора, сделанные на главной, не переносятся на `/deals`. Если на главной был открыт новый предпросмотр или пришла подсветка по SSE, они остаются активными после перехода.
