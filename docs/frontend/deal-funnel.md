...
4. **Панель массовых действий (`DealBulkActions`):** фиксируется снизу при выборе ≥1 сделки, но в текущей версии отображает заглушку с кнопками и сообщением о переносе фактических операций в этап 1.1. Реализация API и UX для действий («Назначить менеджера», «Изменить этап», «Добавить задачу», «Удалить») планируется в рамках обновления `DealBulkActions` на этапе 1.1.
...
- **Главная страница.** Виджет обёрнут в `HomeDealFunnelBoard`, который при монтировании делает снимок состояния `/deals`, затем вызывает `clearFilters()` для сброса до `createDefaultDealFilters()`, очищает `selectedDealIds`, закрывает предпросмотр и снимает подсветку, чтобы на `/` всегда отображалась дефолтная канбан-доска (`forceViewMode="kanban"`). При размонтировании компонент сравнивает текущее состояние с полученным после сброса: если пользователь оставил его без изменений, возвращаются исходные фильтры, выделение, предпросмотр и подсветка. Любые новые фильтры, выбранные сделки, открытый предпросмотр или подсветки (например, из потока событий CRM или после повторного REST-запроса), сделанные на главной, остаются активными после перехода обратно на `/deals`.
...
- **Массовые действия:** чекбоксы на карточках синхронизируются между канбаном и таблицей. Пока операции не подключены: панель выводит заглушку и информирует о переносе реализации на этап 1.1, после подтверждения пользователем выделение очищается.
...
- **Настройки окружения:** переменная `NEXT_PUBLIC_API_BASE_URL` должна указывать на Gateway (`http://gateway:8080/api`) или иметь значение `mock` для полностью локального режима без бэкенда. Таймауты запросов задаются переменными `FRONTEND_PROXY_TIMEOUT` (браузер, middleware) и `FRONTEND_SERVER_TIMEOUT_MS` (SSR).
...
- **Главная страница (`/`) и `/deals`:** страницы больше не префетчат сделки и метрики на сервере. Компоненты `DealFunnelBoard`, `DealFunnelHeader` и `DealFunnelTable` самостоятельно выполняют запросы через React Query на клиенте и показывают скелетоны (`BoardSkeleton`, placeholders таблицы), поэтому первый рендер страницы не блокируется ожиданием REST-ответов.
...
- **Главная ↔ страница сделок.** `HomeDealFunnelBoard` делает снимок состояния `/deals`, временно сбрасывает его до дефолта для домашнего виджета и при размонтировании восстанавливает сохранённые значения только если пользователь их не менял. Поэтому фильтры, выбранные сделки, предпросмотр и подсветка, изменённые на главной, переносятся в UI-стор и остаются активными после перехода на `/deals`, а при отсутствии взаимодействия возвращается состояние до визита.
