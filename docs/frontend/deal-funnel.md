...
- **Главная страница.** `DealFunnelBoard` получает проп `forceViewMode="kanban"`, поэтому виджет всегда рендерится в канбан-режиме независимо от переключателя в UI-сторе. Это гарантирует наличие визуализации воронки сразу после входа в систему.
