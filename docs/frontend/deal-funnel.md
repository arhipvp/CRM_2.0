...
3. **Табличный режим:** компактный список сделок с теми же данными, отдельной колонкой «Следующий просмотр» и сортировкой по ближайшей дате (фолбэк — `updatedAt`, более свежие изменения выше).
...
- **Сортировка.** `filters.sort` по умолчанию = `nextReviewAt` и синхронизирован с параметром API `next_review_at`; значения в прошлом отображаются выше, `null` опускаются вниз, затем применяется `updatedAt` по убыванию.
