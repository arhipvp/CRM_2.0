...
- **Главная страница.** Виджет обёрнут в `HomeDealFunnelBoard`, который принудительно сбрасывает фильтры и выбор в состояние `createDefaultDealFilters()` и закрывает предпросмотр, поэтому на `/` всегда отображается дефолтная сводка без утечки состояния обратно на `/deals`. Сброс выполняется в изоморфном `useLayoutEffect`, поэтому React блокирует первый кадр до применения дефолтных фильтров, массового выделения и предпросмотра — пользователь не видит промежуточных состояний, а `useDeals` не делает повторный запрос. В cleanup компонент возвращает сохранённый снэпшот UI-store и по-прежнему передаёт `forceViewMode="kanban"`, чтобы вид оставался канбаном независимо от переключателя в UI-сторе.
- **Страница `/deals`.** Поведение остаётся управляемым через глобальный `viewMode`, позволяя пользователю переключаться между канбаном и таблицей.
...
- **env.example:** актуален и продолжает содержать полный перечень необходимых переменных, дополнительных обновлений не требуется.
