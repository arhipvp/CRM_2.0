...
- **Создание сделки:** кнопка «Новая сделка» в хедере открывает модальное окно с обязательными полями «Название сделки», «Клиент», «Следующий просмотр» и опциональными «Ответственный», «Описание». Над выпадающим списком клиентов добавлено поле поиска с фильтрацией по префиксу (без учёта регистра), при этом уже выбранный клиент остаётся в списке, даже если не соответствует фильтру. После сохранения модалка закрывается, в хедере появляется toast «Сделка создана», а списки сделок и метрики стадий инвалидируются.
