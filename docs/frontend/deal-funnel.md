...
- **Создание сделки:** кнопка «Новая сделка» в хедере открывает модальное окно с обязательными полями «Название сделки», «Клиент», «Следующий просмотр» и опциональными «Ответственный», «Описание». После сохранения модалка закрывается, в хедере появляется toast «Сделка создана», а списки сделок и метрики стадий инвалидируются.
