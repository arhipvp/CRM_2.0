...
- Редактирование поля разрешено только в режиме «Редактирование»; при сохранении отправляется PATCH-запрос к `/deals/{id}` с полем `nextReviewAt` (camelCase) и автоматическим обновлением списка сделок.
