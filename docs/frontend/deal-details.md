...
- Поле `nextReviewAt` находится в «Основных данных» и используется как дата/время с выбором из календаря и быстрых пресетов («Сегодня 18:00», «Завтра 10:00», «+7 дней»).
...
| Просмотр | Используется по умолчанию для пользователей с правами чтения. | Заблокированы элементы редактирования; отображается кнопка «Запросить изменение». |
...
- При очистке поля карточка теряет бейдж, перемещается в конец списков, отсортированных по `nextReviewAt`, и выпадает из блока «Повестка на сегодня».
