...
- **Навигация по вкладкам:** горизонтальный таб-контрол, позволяющий переключаться между «Обзор», «Формы», «Полисы», «Журнал» и «Действия».
...
- Для каждого полиса отображается вложенный список платежей с колонками «Номер», «Тип операции» (доход/расход), «Плановая дата», «Фактическая дата», «Сумма», «Статус подтверждения» и «Ответственный».
- Сводная строка под таблицей агрегирует суммы по категориям: «Всего начислено», «Получено», «Расходы исполнителя» и «Комиссия продавца».
- При наличии более трёх записей список сворачивается в пагинированный блок с быстрыми фильтрами «Все», «Доходы», «Расходы», «Просроченные».
...
- Каждая строка платежа может разворачиваться в подробности: источник дохода (например, «Клиент», «Партнёр»), статья расхода («Медосмотр», «Диагностика») и привязанные документы.
- При раскрытии отображаются таймлайн согласований и история изменений (`createdAt`, `updatedAt`, пользователь, комментарий).
- Для доходов выводится блок «Разнесение по счетам» с реквизитами и статусом сверки, для расходов — «Компенсация» с полем «Кто одобрил».
- Состояния раскрытия поддерживают быстрые теги: «Ожидает подтверждения», «Частичный платёж», «Закрыт».

#### Действия с платежами
- Доступные действия по правам: продавцы могут добавлять/редактировать доходы, исполнители — расходы; администраторы видят оба сценария.
- Кнопка «Добавить платёж» открывает модальное окно с полями «Тип» (доход/расход), «Категория», «Плановая дата», «Сумма», «Комментарий» и чекбоксом «Пометить как подтверждённый» (доступно только при наличии фактической даты).
- Редактирование запускается из меню строки, сохраняет историю версий и требует указать причину изменения (выпадающий список + текстовое поле).
- Удаление доступно, если платёж не подтверждён: система требует двойное подтверждение и фиксирует событие в журнале сделки.
- Все CRUD-действия отображают всплывающие уведомления и синхронизируются с SSE-каналом CRM для мгновенного обновления списка у всех пользователей.
