...
- **Критерии приёмки:**
  - Сделка обновляется без перезагрузки, отображается в таблице в соответствии с активными фильтрами.
  - Строки сортируются по `nextReviewAt`: просроченные обзоры закреплены сверху, остальные идут по возрастанию даты, поле всегда заполнено.
...
- **Доступность:**
  - Таблица сделок управляется клавиатурой, состояние строк и заголовков озвучивается.
