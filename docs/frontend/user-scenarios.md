...
- **Последовательность:**
  ...
  2. Переключиться на вкладку «Документы», загрузить исходные файлы и добавить заметку в журнал.
  3. На вкладке «Расчёты» создать новую запись: выбрать страховую компанию, программу, заполнить премию, период действия и приложить рабочие файлы.
  4. Сохранить расчёт в статусе «Черновик», проверить авто-валидацию обязательных полей и исправить ошибки (если есть).
  5. После проверки данных нажать «Отметить как готово», чтобы перевести расчёт в статус «Готов» и уведомить назначенного менеджера.
  6. После выбора клиентом варианта обновить детали полиса и статус подготовки в блоке быстрого редактирования или через вкладку «Полисы» (доступ по обновлённым правам роли).
  7. Зафиксировать расходы на обследование и доставку документов, прикрепить подтверждающие файлы и отправить на компенсацию.
  8. Закрыть задачу, указав фактическое время и приложив итоговые файлы.
- **Критерии приёмки:**
  ...
  - При создании расчёта все обязательные поля валидируются, ошибки отображаются под соответствующими инпутами и блокируют сохранение до исправления.
  - Расчёт в статусе «Готов» автоматически связывается с выбранным полисом или помечается как «не привязан», ссылка корректно открывает карточку полиса в новом табе.
  - Перевод расчёта в статус «Готов» добавляет событие в журнал сделки и уведомление менеджеру, а файлы остаются доступными для скачивания.
