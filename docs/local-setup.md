...
2. После синхронизации проверьте секреты и уникальные параметры вручную:
   ...
   3. Повторите проверку для `.env` каждого сервиса, который был скопирован или перезаписан, чтобы не оставить дефолтные секреты.
      ...
      - Каталог `backend/payments` сохранён только как архивная документация: отдельная схема `payments` и переменные `PAYMENTS_DB_*` больше не используются, а `.env` для этого сервиса не синхронизируется автоматически. Для редких легаси-сценариев оставлена пара переменных `PAYMENTS_RABBITMQ_URL`/`PAYMENTS_RABBITMQ_INTERNAL_URL`, чтобы можно было подключить сервис к брокеру (см. `env.example`).
...
5. **Инициализация.** После получения токена пропишите его в `TELEGRAM_BOT_BOT_TOKEN`, секрет подписи webhook'а — в `TELEGRAM_BOT_WEBHOOK_SECRET`. Публичный URL настраивается на стороне Gateway/BFF (см. [backend/telegram-bot/README.md](../backend/telegram-bot/README.md)).
