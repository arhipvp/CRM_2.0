...
> Скрипт `./scripts/load-seeds.sh` работает только с SQL-файлами `backups/postgres/seeds` и не требует отдельных шагов для standalone-сервиса задач: структура схемы `tasks` обслуживается CRM и Alembic-миграциями.
