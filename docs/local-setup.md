...
9. `docker compose --profile backend wait` (либо резервный опрос healthcheck) — ожидание готовности сервисов профиля. Для пропуска передайте `--skip-backend-wait` или `BOOTSTRAP_SKIP_BACKEND_WAIT=true`.
