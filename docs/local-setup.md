...
Bootstrap-скрипт выполняет полный цикл подготовки инфраструктуры и автоматически поднимает фронтенд. Перед запуском Docker Compose он импортирует переменные из актуального `.env` без дополнительной обработки: шаблон `env.example` теперь содержит развёрнутые значения вместо ссылок вида `VAR=${OTHER_VAR}`. После изменения `.env` перезапустите `./scripts/bootstrap-local.sh`, чтобы обновлённые значения попали в контейнеры и дочерние процессы. При необходимости запустить прикладные сервисы на хосте добавьте флаг `--with-backend` или переменную `BOOTSTRAP_WITH_BACKEND=true` — после миграций helper `scripts/start-backend.sh` создаст фоновые процессы, сохранив PID/логи в `.local/run/backend` и проверив отсутствие дублирующих запусков.
...
При запуске `bootstrap-local.sh` скрипт пытается зарезервировать порты, указанные в `.env` (в первую очередь `POSTGRES_PORT`, который пробрасывается наружу). Контейнер PostgreSQL всегда слушает `POSTGRES_CONTAINER_PORT=5432`, поэтому внутренний порт менять не нужно — достаточно подобрать свободное значение `POSTGRES_PORT`, если 5432 занят на хосте. Проверка прерывает выполнение с сообщением вида «Порт 5432, заданный переменной POSTGRES_PORT в .env, уже используется», после чего нужно обновить только `POSTGRES_PORT` и перезапустить bootstrap. На системах без IPv6 (например, Windows с отключённым стеком) скрипт пропускает попытку `bind` на `::` и игнорирует ошибку `WSAEINVAL`, чтобы не создавать ложных срабатываний.
При запуске `bootstrap-local.sh` скрипт пытается зарезервировать порты, указанные в `.env` (в первую очередь `POSTGRES_PORT`). Если обнаружен конфликт, выполнение прерывается с сообщением вида «Порт 5432, заданный переменной POSTGRES_PORT в .env, уже используется». В этом случае выберите свободное значение порта в файле `.env` и перезапустите bootstrap. По необходимости обновите связанные переменные в шаблоне `env.example` и дочерних сервисах. На хостах без IPv6 проверка пропускает попытку `bind` на `::`, чтобы не создавать ложных срабатываний. Дополнительно учтён кейс Windows с отключённым IPv6, где при попытке забиндить `::` система возвращает `WSAEINVAL`; скрипт воспринимает такую ошибку как сигнал продолжить проверку на IPv4, поэтому шаг «Проверка портов .env» завершается успешно.
...
`scripts/start-backend.sh` проверяет наличие `pnpm`, `poetry` и `java`, импортирует `.env`, затем запускает Auth (`./gradlew bootRun`), CRM API (`poetry run crm-api`), CRM worker (`poetry run crm-worker worker -l info`) и Gateway (`pnpm start:dev`) в фоне. PID-файлы, журналы сервисов и журнал запуска по умолчанию размещаются в `.local/run/backend/{pids,logs,start-backend.log}`; повторный запуск выявляет уже работающие процессы, не дублирует их и дописывает сообщения в общий лог. Путь можно переопределить через опцию `--log-file PATH` или переменную `START_BACKEND_LOG_FILE`. Для завершения процессов используйте `./scripts/stop-backend.sh` — скрипт читает PID-файлы, отправляет `SIGTERM` (при необходимости `SIGKILL`) и очищает служебные файлы. Опция `--log-file PATH` (или та же переменная `START_BACKEND_LOG_FILE`) позволяет явно указать журнал запуска при остановке. При флаге `--clean-logs` helper удаляет журналы сервисов и файл запуска (удаление возможно только для путей внутри каталога `.local/run/backend`); в остальных случаях логи остаются для отладки и пополняются при следующем запуске.
