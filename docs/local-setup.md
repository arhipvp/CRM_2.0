...
> ℹ️ Веб-интерфейс CRM (Next.js) разворачивается контейнером `frontend`, который входит в compose-профиль backend. Bootstrap отслеживает его готовность вместе с Gateway/Auth/CRM/Documents, а UI доступен по `http://localhost:${FRONTEND_PORT:-3000}`.
...
Сразу после синхронизации `.env` запускается проверка `./scripts/check-env.sh`: скрипт читает шаблон `env.example`, сверяет обязательные переменные (`POSTGRES_*`, `*_DATABASE_URL`, `BACKUP_RABBITMQ_ADMIN_*`, `CONSUL_*`, `RABBITMQ_*`, ключевые URL сервисов) и сообщает о плейсхолдерах `change_me`, `ChangeMePlease123!`, dev-токенах Telegram и пустых значениях. Запускайте его вручную перед `docker compose`, если правите `.env` вне bootstrap — так ошибки переменных будут найдены до старта контейнеров. Для проверки всех сервисных конфигов используйте `./scripts/check-env.sh --all`.
...
Bootstrap также синхронизирует пароли PostgreSQL-ролей с текущими значениями переменных окружения при каждом запуске: если вы обновили секреты в `.env`, просто перезапустите скрипт, и роли получат новые пароли без ручных действий. Дополнительно `infra/postgres/init.sh` пересоздаёт роли с правами `LOGIN`/`CREATEDB`, выставляет `GRANT CREATE` на базу и схемы (`auth`, `crm`, `documents`, `backup`, `reports`) и настраивает перекрёстные права CRM, включая схему `tasks`.

> ⚠️ Скрипты инициализации PostgreSQL выполняются только при первом запуске контейнера с пустыми данными. Если требуется повторно применить обновления (например, после правок `infra/postgres/init.sh`), остановите инфраструктуру командой `docker compose down -v --remove-orphans` из каталога `infra/`, чтобы удалить тома, и затем запустите `./scripts/bootstrap-local.sh` заново.
...
- `--skip-backend-build` — передаёт в bootstrap флаг пропуска пересборки образов профиля backend (`DEV_UP_SKIP_BACKEND_BUILD=true`);
...
|                       | `--skip-backend-build` / `BOOTSTRAP_SKIP_BACKEND_BUILD=true` | Пропустить пересборку образов профиля backend |
...
|                       | `--skip-backend-build` / `DEV_UP_SKIP_BACKEND_BUILD=true` | Передать пропуск пересборки образов профиля backend |
...
1. `scripts/sync-env.sh --non-interactive` — создание отсутствующих `.env` во всех сервисах по шаблону `env.example` без ожидания ввода; существующие файлы пропускаются, поэтому для обновления значений заранее выполните `scripts/sync-env.sh --non-interactive=overwrite` или отредактируйте их вручную.
2. `./scripts/check-env.sh --env-file .env` — проверка обязательных переменных, поиск плейсхолдеров `change_me`/`ChangeMePlease123!` и незаполненных `BACKUP_RABBITMQ_ADMIN_*`, `POSTGRES_*`, `*_DATABASE_URL` до старта Docker Compose.
3. `docker compose --env-file .env up -d` в каталоге `infra/` — запуск PostgreSQL, RabbitMQ, Redis и вспомогательных сервисов с подстановкой значений из актуального корневого `.env`.
   > ℹ️ Если в логах остаются только Redis/Consul/Postgres, выполните `docker compose --env-file .env --profile backend up --build -d` для запуска профиля backend; без необходимости пересборки можно опустить `--build`.
4. Ожидание готовности инфраструктуры через healthcheck (`docker compose wait` либо резервный цикл при отсутствии команды).
...
6. `scripts/migrate-local.sh` — миграции CRM (Alembic), Auth (Liquibase/Gradle) и Reports (SQL через `psql`), пока backend-профиль ещё не запущен.
7. Smoke-проверку `BACKUP_*`: шаг загружает `.env`, собирает значения `BACKUP_S3_*` и, если все они пусты, фиксирует использование `DummyStorage` без ошибок. Когда хотя бы одна переменная заполнена, скрипт требует полный набор (`BACKUP_S3_BUCKET`, `BACKUP_S3_ACCESS_KEY`, `BACKUP_S3_SECRET_KEY`) и завершает bootstrap с ошибкой, если обнаружены пропуски.
8. Дополнительную smoke-проверку: при полностью пустых `BACKUP_S3_*` шаг проверяет, что контейнер `backup` работает (статус `running`) и логи содержат предупреждение о `DummyStorage`. Если найдена любая заполненная переменная `BACKUP_S3_*`, проверка завершается сразу — предполагается настройка S3 и диагностика DummyStorage не нужна.
9. `docker compose --env-file .env --profile backend up --build -d` — старт профильных сервисов (Gateway, Auth, CRM, Documents, Frontend) после применения миграций. Скрипт поддерживает флаг `--skip-backend` (или переменную `BOOTSTRAP_SKIP_BACKEND=true`), если требуется ограничиться инфраструктурой без прикладных сервисов.
...
10. Ожидание готовности сервисов профиля backend через helper `check_backend_services`, который анализирует `docker compose --profile backend ps --format json` либо табличный вывод `docker compose ps`. Шаг проверяет статусы Gateway/Auth/CRM/Documents и доступность Frontend; для пропуска передайте `--skip-backend-wait` или `BOOTSTRAP_SKIP_BACKEND_WAIT=true`.
11. `scripts/start-backend.sh` — запуск прикладных сервисов на хосте (Auth `./gradlew bootRun`, CRM API `poetry run crm-api`, CRM worker `poetry run crm-worker worker -l info`, Gateway `pnpm start:dev`). Шаг активируется только при комбинации `--with-backend --skip-backend` или переменных `BOOTSTRAP_WITH_BACKEND=true` и `BOOTSTRAP_SKIP_BACKEND=true`; PID, журналы сервисов и файл запуска по умолчанию сохраняются в `.local/run/backend`, повторный запуск проверяет наличие активных процессов. Опция `--service NAME` у `scripts/start-backend.sh` позволяет стартовать только выбранные сервисы (можно перечислять имена через запятую или повторять опцию для добавления новых значений).
...
14. Проверку REST/SSE API backend-профиля: Gateway (`/api/v1/health`, `/api/v1/streams/heartbeat`), Auth (`/actuator/health`), CRM (`/healthz`), Documents (`/health`) и главной страницы Frontend (`/`).

Для ручного выполнения шагов bootstrap (например, для отладки) используйте `./scripts/bootstrap-local.sh`. Для профиля backend используйте `docker compose --env-file .env --profile backend up --build -d` либо `down` для отключения; при пропуске пересборки достаточно `docker compose --profile backend up -d`. После ручного редактирования `.env` обязательно перезапустите bootstrap, чтобы переменные снова экспортировались и попали в Docker Compose.

`scripts/start-backend.sh` проверяет наличие `pnpm`, `poetry` и `java`, импортирует `.env`, затем запускает Auth (`./gradlew bootRun`), CRM API (`poetry run crm-api`), CRM worker (`poetry run crm-worker worker -l info`) и Gateway (`pnpm start:dev`) в фоне. Перед стартом CRM-сервисов helper убеждается, что для каталога `backend/crm` уже создано Poetry-окружение, и при необходимости выполняет `poetry install --sync --no-root`, поэтому дополнительный ручной `poetry install` перед использованием комбинации `--with-backend --skip-backend` не требуется. PID-файлы, журналы сервисов и журнал запуска по умолчанию размещаются в `.local/run/backend/{pids,logs,start-backend.log}`; повторный запуск выявляет уже работающие процессы, не дублирует их и дописывает сообщения в общий лог. Опция `--service NAME` в `scripts/start-backend.sh` фильтрует список запускаемых сервисов (значения валидируются относительно известного набора и не допускают дублирования), путь можно переопределить через `--log-file PATH` или переменную `START_BACKEND_LOG_FILE`. Например, чтобы ограничиться Gateway и CRM API, выполните `scripts/start-backend.sh --service gateway,crm-api`. Для завершения процессов используйте `./scripts/stop-backend.sh` — скрипт читает PID-файлы, отправляет `SIGTERM` (при необходимости `SIGKILL`) и очищает служебные файлы. Флаг `--service NAME` принимает одно имя или список через запятую (`--service gateway,crm-api`) и допускает повторение ключа для добавления сервисов по одному (`--service gateway --service crm-worker`). Это позволяет целенаправленно останавливать только часть процессов, не затрагивая остальные, что удобно при пошаговой отладке и ручном перезапуске отдельных сервисов. Опция `--log-file PATH` (или та же переменная `START_BACKEND_LOG_FILE`) позволяет явно указать журнал запуска при остановке. При флаге `--clean-logs` helper удаляет журналы сервисов и файл запуска (удаление возможно только для путей внутри каталога `.local/run/backend`); в остальных случаях логи остаются для отладки и пополняются при следующем запуске.

> ℹ️ Все инфраструктурные скрипты и команды Docker Compose теперь вызываются с `--env-file .env`, поэтому важно поддерживать корневой `.env` в актуальном состоянии: `scripts/sync-env.sh --non-interactive` добавляет только отсутствующие файлы, а для перезаписи существующих используйте `--non-interactive=overwrite` или обновляйте значения вручную.
...
> 💡 Desktop-приложение использует переменную `DESKTOP_API_BASE_URL`. При работе через Gateway указывайте путь с `/api/v1/crm` (например, `http://localhost:8080/api/v1/crm`); без суффикса `/crm` приложение обратится в корень Gateway и не найдёт CRM-эндпоинты. Значение `http://localhost:8082/api/v1` подходит только для прямого подключения к CRM.

### CRM / Deals: быстрый старт

- Перейдите в `backend/crm` и, если запускаете сервис вручную, установите зависимости `poetry install --sync --no-root`. При запуске через `./scripts/bootstrap-local.sh --with-backend --skip-backend` helper `start-backend.sh` подтянет зависимости автоматически.
...
- Примените миграции: `poetry run alembic upgrade head`.
  ...
  > автоматически создают схему при отсутствии и переназначают владельца на текущего
  > пользователя, поэтому достаточно выдать временное право `ALTER` текущей роли.
...
2. Обновите в `.env` чувствительные значения:
   ...
   - Обновите `BACKUP_DATABASE_URL` на формат `?options=-csearch_path%3Dbackup` (как в `env.example`). Старый параметр `?search_path=backup` больше не поддерживается libpq в Kubernetes, поэтому секреты окружений нужно пересобрать с новым URI.
...
> Скрипт `./scripts/load-seeds.sh` работает только с SQL-файлами `backups/postgres/seeds` и больше не обращается к удалённому легаси-сервису Tasks, поэтому дополнительный `pnpm` и переменная `TASKS_DATABASE_URL` исключены из шаблона окружения.
...
#### Автоматическая инициализация на VPS

Для серверов без Docker Compose используйте `scripts/bootstrap-db-vps.sh`. Скрипт читает значения пользователей и паролей из `.env`, поэтому перед запуском синхронизируйте файл с `env.example` и обновите секреты вручную.

**Требования**

- Доступ к целевому экземпляру PostgreSQL (обычно через SSH-туннель или прямое подключение);
- установленный `psql` на рабочей машине;
- актуальный `.env` в корне репозитория, содержащий блоки `POSTGRES_*`, `AUTH_*`, `CRM_*`, `DOCUMENTS_*`.

**Ключевые флаги**

- `--env-file PATH` — путь к файлу окружения, из которого будут считаны параметры подключения и учетные данные ролей (по умолчанию `./.env`);
- `--admin-db URL` — строка подключения администратора PostgreSQL (например, `postgresql://postgres:...@db-host:5432/postgres`), через которую выполняется настройка.

**Что делает скрипт**

- создаёт базы данных, перечисленные в `.env` (Auth, CRM, Documents);
- включает требуемые расширения (`pgcrypto`, `uuid-ossp`, `pg_trgm` и т. п.);
- создаёт роли приложений с паролями из `.env` и назначает им права;
- разворачивает схемы (`auth`, `crm`, `documents`, `tasks`, `notifications`, `backup`) и устанавливает владельцев.

Запускайте `scripts/bootstrap-db-vps.sh`, когда разворачиваете PostgreSQL на VPS или выделенном сервере без инфраструктурных контейнеров: скрипт повторяет шаги `infra/postgres/init.sh`, но работает напрямую против удалённой БД.
...
- Для проверки готовности можно запустить ключевые сервисы локально:
  ...
  - **Auth:**
    ...
    > 💡 Для локальной разработки можно включить bootstrap администратора: установите в `.env` `AUTH_BOOTSTRAP_ENABLED=true`, задайте email (`AUTH_BOOTSTRAP_EMAIL`), временный пароль (`AUTH_BOOTSTRAP_PASSWORD`) и список ролей (`AUTH_BOOTSTRAP_ROLES`, например `ROLE_ADMIN,ROLE_USER`). При старте Auth создаст пользователя, если его ещё нет, и назначит указанные роли.【F:backend/auth/src/main/kotlin/com/crm/auth/bootstrap/AdminBootstrapRunner.kt†L21-L68】 После успешного входа отключите bootstrap (верните `AUTH_BOOTSTRAP_ENABLED=false`) и при необходимости смените пароль через API.

    > ⚠️ **Не используйте bootstrap в продакшене без ротации паролей.** Статический пароль из переменной `AUTH_BOOTSTRAP_PASSWORD` подходит только для первоначальной инициализации. В боевых средах включайте bootstrap один раз с уникальным паролем, сразу меняйте его и выключайте флаг, чтобы исключить повторное применение сценария.
