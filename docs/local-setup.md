...
2. После синхронизации проверьте секреты и уникальные параметры вручную:
   ...
   3. Повторите проверку для `.env` каждого сервиса, который был скопирован или перезаписан, чтобы не оставить дефолтные секреты.
      ...
      - Сервис Payments снова поставляется отдельно (`backend/payments`). Скрипт синхронизирует его `.env`, а Postgres и Docker Compose используют переменные `PAYMENTS_DB_*`, `PAYMENTS_R2DBC_URL`, `PAYMENTS_DATABASE_URL` и их `_INTERNAL`-аналоги.
      - `PAYMENTS_CRM_WEBHOOK_SECRET` остаётся опциональным: задайте секрет только если CRM обрабатывает вебхуки из Payments.
