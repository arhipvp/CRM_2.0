...
6. Smoke-проверку `BACKUP_*`: шаг загружает `.env`, собирает значения `BACKUP_S3_*` и, если все они пусты, фиксирует использование `DummyStorage` без ошибок. Когда хотя бы одна переменная заполнена, скрипт требует полный набор (`BACKUP_S3_BUCKET`, `BACKUP_S3_ACCESS_KEY`, `BACKUP_S3_SECRET_KEY`) и завершает bootstrap с ошибкой, если обнаружены пропуски.
7. Дополнительную smoke-проверку: при полностью пустых `BACKUP_S3_*` шаг проверяет, что контейнер `backup` работает (статус `running`) и логи содержат предупреждение о `DummyStorage`. Если найдена любая заполненная переменная `BACKUP_S3_*`, проверка завершается сразу — предполагается настройка S3 и диагностика DummyStorage не нужна.
8. `docker compose --env-file .env --profile backend up --build -d` — старт профильных сервисов (Gateway, Auth, CRM, Documents, Frontend) после применения миграций. Скрипт поддерживает флаг `--skip-backend` (или переменную `BOOTSTRAP_SKIP_BACKEND=true`), если требуется ограничиться инфраструктурой без прикладных сервисов.
...
13. Проверку REST/SSE API backend-профиля: Gateway (`/api/v1/health`, `/api/v1/streams/heartbeat`), Auth (`/actuator/health`), CRM (`/healthz`), Documents (`/health`) и главной страницы Frontend (`/`).
