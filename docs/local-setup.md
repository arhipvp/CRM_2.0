...
6. `scripts/migrate-local.sh` — миграции CRM (Alembic), Auth и Audit (Liquibase/Gradle) и Reports (SQL через `psql`). Во время bootstrap роль `REPORTS_DB_USER` и схема `${REPORTS_SCHEMA}` создаются в `infra/postgres/init.sh`, поэтому миграции Reports применяются без дополнительных ручных шагов.
...
9. `scripts/check-local-infra.sh` — smoke-проверку PostgreSQL, Redis, Consul, RabbitMQ Management UI, схемы `${REPORTS_SCHEMA}` (через `psql SELECT current_schema`) и /health Reports (при запущенном сервисе).
