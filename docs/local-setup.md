...
2. После синхронизации проверьте секреты и уникальные параметры вручную:
   ...
   3. Повторите проверку для `.env` каждого сервиса, который был скопирован или перезаписан, чтобы не оставить дефолтные секреты.
      ...
      - Модуль платежей CRM принимает события из внешних систем через защищённый webhook (`/payments/webhook`). Значение `PAYMENTS_CRM_WEBHOOK_SECRET` обязательно: в разработке используйте понятный плейсхолдер (например, `local-payments-webhook-secret`), а в продакшене — уникальный 32-символьный токен из секретного хранилища.
