...
- **CRM/Deals** — клиенты, сделки, расчёты, полисы, журнал активности и задачи первого уровня (ручное назначение, без SLA и автоматических сценариев). Учёт оплат полностью ведётся внутри CRM: для сделки хранится единственное поле фактической даты оплаты, которое продавец обновляет вручную на соответствующем этапе.
...
- Продавец и исполнитель фиксируют все типы платежей (премии, комиссии, скидки, выплаты) в рамках своих сделок: у каждого этапа допускается только одна запись платежа, фактическая дата оплаты хранится в единственном поле и вводится продавцом вручную.
