...
- **Notifications (ядро)** — генерация внутренних уведомлений и базовые push/SSE для фронтенда; интеграция с Telegram ограничена событиями «новая задача» и «изменение статуса платежа».
...
2. **Этап 2.0**
   ...
   - Полноценная система уведомлений для клиентов (email/SMS шлюзы).
3. **Этап 3.0**
   ...
   - Введение конструкторов бизнес-процессов для задач и сделок с настраиваемыми этапами согласования.
...
- Telegram-бот поддерживает только уведомления без интерактивных сценариев (создание сделки, подтверждения операций — в roadmap Этапа 1.1).
