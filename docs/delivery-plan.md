...
- **CRM/Deals** — клиенты, сделки, расчёты, полисы, журнал активности и задачи первого уровня (ручное назначение, без SLA и автоматических сценариев). Учёт платежей ведётся внутри сервиса CRM: у сделки одна запись платежа на этап, а дату фактической оплаты продавец вводит вручную.
...
- Продавец и исполнитель фиксируют все типы платежей (премии, комиссии, скидки, выплаты) в рамках своих сделок: у каждого этапа допускается только одна запись платежа, дата фактической оплаты вводится продавцом вручную.
...
- SSE-каналы для фронтенда (Gateway) работают для событий сделок и внутренних уведомлений; данные по платежам фронтенд получает через общие REST-запросы CRM. Отдельный поток задач отложен до последующих релизов, smoke-тесты текущих каналов проходят успешно.
