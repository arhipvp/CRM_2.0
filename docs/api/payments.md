...
> ℹ️ API обслуживается модулем платежей внутри CRM/Deals. Каталог [`backend/payments`](../../backend/payments/README.md) хранит архивную документацию прежнего standalone-сервиса и не входит в текущую поставку; используйте его только как справку по легаси-материалам.
...
| actual_date | date | Нет | Дата фактического закрытия платежа; должна быть не раньше `planned_date` (если задана) и не позже текущего дня. |
| recorded_by_id | UUID | Нет | Пользователь, вручную подтвердивший закрытие платежа; передавайте вместе с `actual_date`, если фиксируется конкретный сотрудник. |
| created_by_id | UUID | Нет | Пользователь, от имени которого создаётся платёж (если не передан, CRM использует текущего пользователя). |

> ℹ️ Агрегированные поля `incomes_total`/`expenses_total`/`net_total` передавать не нужно — CRM пересчитывает их автоматически после каждой операции.
> Для корректировки сумм используйте маршруты `/incomes` и `/expenses`: после добавления, изменения или удаления операций CRM пересчитает агрегаты и статус платежа.
...
> ℹ️ Ручное изменение агрегатов не поддерживается. Обновляйте поступления и расходы через соответствующие операции, после чего CRM автоматически синхронизирует суммы и статус платежа.
