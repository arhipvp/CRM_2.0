...
> Направление движения средств определяется вызываемым endpoint'ом: `.../incomes` возвращает и принимает только поступления, а `.../expenses` — только расходы.
...
Обновляет плановые значения, комментарий и дату закрытия платежа. Если указано `actual_date`, CRM обновит агрегаты и статус в соответствии с движением средств.
