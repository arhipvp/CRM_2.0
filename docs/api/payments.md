...
Обновляет плановые значения, комментарий и дату закрытия платежа. Если указано `actual_date`, CRM сохранит дату и обновит `updated_by_id` текущего пользователя.
