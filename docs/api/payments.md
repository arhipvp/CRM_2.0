...
**Ошибки:** `404 payment_not_found` (в теле ответа поле `message` содержит `payment_not_found`).
...
### GET `/payments/export`
Генерация CSV/Excel отчёта по фильтрам (асинхронная операция). Поддерживает те же параметры, что и `/payments`, а также обязательный `format` (`csv` или `xlsx`). Фильтры по статусу и типу можно передавать как одиночными (`status`, `type`), так и массивами (`statuses`, `types`) — сервис приведёт значения к спискам перед публикацией задачи.

**Ответ 202** — задание поставлено в очередь экспорта (`payments.exports`, переопределяется переменной `PAYMENTS_EXPORTS_QUEUE`).
...
Сервис сохраняет параметры фильтрации и публикует сообщение воркеру выгрузок, передавая настройки хранилища из `PAYMENTS_EXPORTS_STORAGE_*`. Первичный статус всегда `processing` — он сменится после обработки события статуса из очереди `PAYMENTS_EXPORTS_STATUS_QUEUE` (по умолчанию `payments.exports.status`).

### GET `/payments/export/{job_id}`
Проверка статуса выгрузки. Возвращает текущее состояние (`processing`, `done`, `failed`). Когда статус `done`, поле `download_url` содержит готовую ссылку из воркера (учитывает TTL `PAYMENTS_EXPORTS_STORAGE_URL_TTL`). При `failed` ссылка отсутствует, а подробности доступны в логах сервиса экспорта.
...
**Ошибки:** `404 export_not_found` — задание не найдено.
