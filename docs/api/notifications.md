...
Возвращает текущее состояние уведомления и агрегированную статистику попыток доставки.

**Параметры пути**
| Параметр | Тип | Обязательный | Описание |
| --- | --- | --- | --- |
| `notification_id` | UUID | Да | Идентификатор уведомления, возвращённый `POST /api/v1/notifications`. |
...
```json
{
  ...
  "status": "queued",
  "attempts": 3,
  "channels": ["rabbitmq", "redis", "events-service"],
  "delivered_at": null
}
```

`status` — см. [таблицу статусов уведомлений](#таблица-статусов-уведомлений). `channels` — см. [таблицу каналов попыток доставки](#таблица-каналов-попыток-доставки).

**Ошибки**
- `401 unauthorized` — отсутствует или просрочен JWT.
- `404 notification_not_found` — уведомление с указанным `notification_id` не найдено или уже удалено из журнала.
...
| --- | --- |
| `pending` | Запись создана и ожидает первой попытки доставки. |
| `queued` | Уведомление успешно отправлено в RabbitMQ и ожидает обработки внешними воркерами. |
| `processed` | Все внутренние шаги обработки выполнены без ошибок; сервис ожидает подтверждений каналов доставки. |
| `failed` | Последняя попытка обработки завершилась ошибкой. Подробности доступны в журнале попыток уведомления. |
| `delivered` | Зарегистрировано подтверждение доставки Telegram (вебхук `delivered`), время указано в `delivered_at`. |

### Таблица каналов попыток доставки

| Канал | Описание |
| --- | --- |
| `rabbitmq` | Постановка задания на доставку в exchange `notifications.exchange`. |
| `redis` | Публикация задания в Redis-канал доставки (`CRM_NOTIFICATIONS_DISPATCH_REDIS_CHANNEL`, по умолчанию `notifications:dispatch`). |
| `events-service` | Создание внутреннего события `NotificationEventIngest` и трансляция в модуль событий CRM. |
