...
`public_url` формируется всегда: сначала сервис использует ссылку, возвращённую драйвером хранилища (если доступна), затем пробует собрать её из `DOCUMENTS_STORAGE_PUBLIC_BASE_URL`, и в качестве резервного варианта использует `DOCUMENTS_FOLDERS_WEB_BASE_URL`. Только явное отключение базового URL приводит к тому, что поле возвращается пустой строкой. Рекомендуем настроить корректное значение `DOCUMENTS_FOLDERS_WEB_BASE_URL`, чтобы при необходимости выдавать рабочие публичные ссылки.
...
**Ответ 200** — `{ "folder_path": "...", "full_path": "...", "public_url": "..." }`. Значение `public_url` заполняется тем же каскадом, что и при создании каталога: драйвер хранилища → `DOCUMENTS_STORAGE_PUBLIC_BASE_URL` → `DOCUMENTS_FOLDERS_WEB_BASE_URL`. Если базовые URL отключены, поле возвращается пустой строкой.
...
Инициирует синхронизацию прав доступа на уровне файловой системы: создаёт запись в `permissions_sync_tasks` и ставит задание `documents.permissions.sync` в очередь BullMQ. На этом ответственность эндпоинта заканчивается — он не применяет ACL напрямую и не управляет системными группами/UID.
...
`job_id` соответствует идентификатору задания BullMQ, `task_id` — записи в таблице `permissions_sync_tasks`. Внесение фактических изменений прав (POSIX-права, ACL, группы) выполняется внешним воркером или ручной обработкой, которая должна забирать задания из очереди и применять настройки к каталогу. TTL задания в очереди регулируется переменной `DOCUMENTS_PERMISSIONS_SYNC_JOB_TTL` (в секундах).

> ⚠️ Сейчас автоматический воркер, применяющий ACL, не реализован. Планируемая функциональность описана в разделе «Фоновая обработка документов»; до её появления операторы должны следить за очередью и запускать обработку вручную.
