...
- **Миграции:** схема `tasks` создаётся TypeORM-миграциями сервиса задач (`pnpm migration:run` в `backend/tasks`), после чего Alembic CRM добавляет внешние ключи и индексы (`poetry run alembic upgrade head`). На чистой базе данные команды выполняются именно в таком порядке, иначе ревизия `2025102601_add_tasks_module` не найдёт созданные таблицы.
