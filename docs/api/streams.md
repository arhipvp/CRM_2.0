...
- Для обратной совместимости внутренний upstream по-прежнему обозначается как `crm`; публичный маршрут и переменные окружения используют имя `deals`, а путь `/api/v1/streams/crm` остаётся рабочим алиасом для клиентов, которые ещё не обновили конфигурацию.【F:backend/gateway/src/sse/sse.controller.ts†L21-L32】
