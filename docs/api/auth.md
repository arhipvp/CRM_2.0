...
### GET `/api/users/{id}`
Возвращает пользователя по идентификатору. Доступно только роли `ADMIN`.

Для формирования ответа используется `UserService.getUserWithRoles`, поэтому возвращаются все базовые поля пользователя и полный список его ролей.
...
| 404 | `User not found` | Пользователь с указанным идентификатором отсутствует. |
