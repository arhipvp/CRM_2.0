...
### GET `/deals`
Возвращает список сделок, отсортированных по `next_review_at` (самые ближайшие проверки первыми) с дополнительной сортировкой по `updated_at`.
...
> Чтобы получить сделки без назначенного менеджера, передайте параметр `manager=__NO_MANAGER__`. Значение можно комбинировать с конкретными `manager=<uuid>` — тогда в выдачу попадут как выбранные владельцы, так и карточки без владельца.
