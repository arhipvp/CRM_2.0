...
- Платёжные события обрабатываются внутри CRM: при создании, обновлении и удалении платежа публикуются `deal.payment.created`, `deal.payment.updated`, `deal.payment.deleted`, а при операциях с движением средств — `deal.payment.transaction.created/updated/deleted` в `crm.events`.
- Внешних очередей для синхронизации с модулем платежей CRM больше нет; повторные попытки редактирования выполняются на уровне REST API, а агрегаты пересчитываются внутри CRM.
