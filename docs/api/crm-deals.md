...
> При обновлении `next_review_at` фронтенд и воронка пересортировывают карточки с учётом новых значений. Передача `null` очищает дату и отправляет сделку в конец списков, отсортированных по обзорам.
