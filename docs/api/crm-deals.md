...
> **Важно:** поле `next_review_at` обязательно — оно определяет позицию сделки в «повестке» списка и карточки деталей. Попытка передать `null` приведёт к ошибке валидации; чтобы изменить порядок, устанавливайте нужную дату обзора.
