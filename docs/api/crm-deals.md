...
> **Важно:** поле `next_review_at` обязательно — оно определяет позицию сделки в рабочем списке и карточке деталей. Попытка передать `null` приведёт к ошибке валидации; чтобы изменить порядок, устанавливайте нужную дату обзора.
