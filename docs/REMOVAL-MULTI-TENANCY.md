...
**Другие сервисы** (auth, gateway, documents, notifications) — НЕ используют tenant_id.

> ℹ️ Модуль задач теперь встроен в CRM. Ранее выделенный сервис Tasks был мигрирован внутрь основного приложения, поэтому дополнительные действия с отдельным сервисом не требуются.
