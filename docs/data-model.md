...
| `auth.roles` | Роли доступа, отражающие доменные обязанности (продавец, исполнитель, главный админ). |
...
| Таблица | Назначение |
| --- | --- |
| `crm.payments` | Единственная запись фактической оплаты по полису сделки без статусов. |
...
Поля таблицы включают сумму, валюту (`RUB`), обязательную фактическую дату (`actual_date`), комментарий и автора подтверждения (`recorded_by_id`). Платёж фиксируется только в `crm.payments` и не имеет статусов: плановые графики и отдельная история изменений не ведутся — правки сохраняются через аудит CRM.
...
| --- | --- |
| `auth` | Справочник ролей (продавец, исполнитель, главный админ). Системный пользователь `system` для фоновых процессов и технический аккаунт бота для интеграций. OAuth-клиент для фронтенда. |
