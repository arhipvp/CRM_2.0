...
* Первичные задачи по подготовке миграций: сформировать начальные ревизии (schema baseline) для схем `auth`, `crm`, `payments`, `documents`, `tasks`, `notifications` и `audit` с фиксацией актуального состояния модели.
