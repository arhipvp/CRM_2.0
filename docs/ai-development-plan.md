...
- ✅ API-клиент фронтенда поддерживает таймауты: `FRONTEND_PROXY_TIMEOUT` управляет браузерными запросами и проксированием Next.js (15 секунд по умолчанию), а `FRONTEND_SERVER_TIMEOUT_MS` задаёт укороченный лимит для SSR (7.5 секунд). Документация и `env.example` отражают оба значения.
