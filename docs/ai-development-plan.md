...
- Сформировать детализированный бэклог первой поставки по сервисам Gateway, Auth, CRM/Deals (FastAPI, включая модули оплат и задач), Documents, Notifications, Backup и пользовательским ролям. Использовать ИИ-инструменты для формирования пользовательских историй и уточнения acceptance-критериев.
...
### Прогресс
- ✅ Автоматизирован быстрый старт окружения: `scripts/bootstrap-local.sh` создаёт отсутствующие `.env`, запускает `docker compose`, настраивает RabbitMQ, применяет миграции, загружает seeds и выполняет smoke-проверку с агрегированным отчётом; документация (`README.md`, `docs/local-setup.md`) дополнена разделами «Быстрый старт» с перечислением зависимостей и последовательности шагов.【F:scripts/bootstrap-local.sh†L1-L133】【F:README.md†L5-L22】【F:docs/local-setup.md†L1-L33】
...
- ✅ Обновления оплат транслируются через канал сделок: публичный маршрут `GET /api/v1/streams/deals` агрегирует события CRM, а документация фиксирует публикацию `deal.payment.updated` вместе с семействами `deal.payment.income.*`/`deal.payment.expense.*` вместо отдельного платёжного потока.【F:backend/gateway/src/sse/sse.controller.ts†L21-L35】【F:docs/api/streams.md†L16-L37】
...
- **CRM/Deals — модуль оплат**: расширить существующий сервис обработкой событий RabbitMQ, Alembic-миграциями и экспортом CSV. ИИ помогает в генерации валидаторов и обработчиков ошибок.
...
- **CRM/Deals — модуль задач**: развивать управление задачами внутри FastAPI-сервиса (SQLAlchemy, Alembic, Celery/RabbitMQ). ИИ используется для генерации расписаний, SLA-проверок и документации будущих расширений.
...
- Подготовить seed-наборы для Auth, CRM/Deals (включая оплату и задачи), Documents, Notifications в `backups/postgres/seeds`. ИИ помогает генерировать репрезентативные данные и сценарии проверки.
