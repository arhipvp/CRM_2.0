...
| `task.status.changed` | `tasks.task.status_changed` | `{ "task_id": "uuid", "old_status": "new", "new_status": "in_progress", "changed_at": "datetime" }` | Повторы определяются по `event_id`. Доступные значения `old_status`/`new_status`: `new` (Новая), `in_progress` (В работе), `done` (Выполнена), `cancelled` (Отменена). |
