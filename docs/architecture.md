...
1. Фронтенд обращается к Gateway/BFF по REST API и поддерживает SSE-каналы для событий, а шлюз определяет целевой сервис и обогащает ответы агрегированными данными.
