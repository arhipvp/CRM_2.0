...
| --- | --- | --- |
| Gateway / BFF | Единая точка входа для веб-клиента и Telegram-бота, оркестрация запросов, управление сессиями, агрегация данных | Redis (сессии, кеш), внутренние REST API сервисов, SSE-каналы CRM, Consul |
...
Отдельного сервиса Payments больше нет: модуль платежей CRM работает внутри CRM/Deals, использует ту же схему `crm`, предоставляет REST API и публикует доменные события `deal.payment.*`. Базовые реквизиты операций лежат в `crm.payments`, детализация доходов и расходов — в таблицах `crm.payment_incomes` и `crm.payment_expenses`; события `deal.payment.updated`, `deal.payment.income.*` и `deal.payment.expense.*` обеспечивают синхронизацию данных для встроенных уведомлений, Reports и Audit. Каталог [`backend/payments`](../backend/payments/README.md) хранит только архивный код standalone-сервиса для анализа миграций.
...
* **Доменные события CRM.** Модуль сделок публикует изменения в exchange `crm.events`, включая созданные сделки, обновления статусов, расчёты, документы и платежи. Модуль задач использует тот же exchange и префиксы маршрутов `task.*`, а модуль уведомлений формирует события `notification.*` для SSE и Telegram.
* **Telegram Bot.** Бот получает команды из очереди `telegram.bot.notifications`, подписанной на `crm.events`, и публикует ответы (например, `crm.deal.created`, `task.status.changed`) в exchange `crm.domain`.
...
* Активные схемы: `auth`, `crm`, `documents`, `audit`.
...
* Предоставляет fallback-сценарии: при недоступности отдельных модулей CRM сообщает о деградации и переключается на кэшированные данные, где это допустимо.
...
3. CRM/Deals создаёт записи в своей схеме PostgreSQL, публикует событие `deal.created` в RabbitMQ и планирует задачу (если требуется) во встроенном модуле задач.
