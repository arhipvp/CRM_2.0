...
2. Documents сохраняет параметры запроса, ставит задание в очередь BullMQ `documents.permissions.sync` и возвращает идентификаторы задачи и записи синхронизации. Сам эндпоинт ничего не делает с ACL — он лишь постановщик.
3. Применение POSIX-прав и ACL выполняет внешний обработчик очереди (воркер BullMQ или ручной процесс администратора), который забирает задания `documents.permissions.sync` и запускается отдельно от API. Инструкции по запуску штатного воркера и ручной обработке очереди приведены в [`backend/documents/README.md`](../backend/documents/README.md#ручная-обработка-очереди-documentspermissionssync).
4. После выполнения задания обработчик обновляет статус в репозитории задач Documents; CRM/Deals запрашивает результат через очередь или REST и обновляет аудит доступа.
