...
> Имя очередей (`queues.documents`, `queues.permissionsSync.name`) сервис получает из `ConfigService` после применения схемы валидации, поэтому значения в `.env` проходят проверку и подставляются вместе со значениями по умолчанию.
