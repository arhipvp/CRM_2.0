...
5. Поднимите Celery-воркер (использует очереди Redis):
   ...
   ```
6. Платёжный модуль работает через вложенные REST-ресурсы `/api/v1/deals/{dealId}/policies/{policyId}/payments` с дополнительными коллекциями `/incomes` и `/expenses`. Дополнительных брокеров сообщений или внешних воркеров не требуется: сервис напрямую обновляет таблицы `crm.payments`, `crm.payment_incomes`, `crm.payment_expenses` и публикует доменные события `deal.payment.*` в `crm.events`.
...
- `GET /api/v1/deals/{deal_id}/policies/{policy_id}/payments` — список платежей полиса (поддерживает `include=incomes,expenses`).
- `POST /api/v1/deals/{deal_id}/policies/{policy_id}/payments` — создать платёж и инициировать событие `deal.payment.created`.
- `GET/PATCH/DELETE /api/v1/deals/{deal_id}/policies/{policy_id}/payments/{payment_id}` — получить, обновить или удалить платёж.
- `GET /api/v1/deals/{deal_id}/policies/{policy_id}/payments/{payment_id}/incomes` — операции поступлений.
- `GET /api/v1/deals/{deal_id}/policies/{policy_id}/payments/{payment_id}/expenses` — операции расходов.
- `POST/PATCH/DELETE` для `/incomes` и `/expenses` — CRUD-потоки для записей, публикующие события `deal.payment.income.*` и `deal.payment.expense.*`.
