...
## Назначение
CRM/Deals управляет клиентами, сделками, полисами, задачами и встроенным модулем платежей. Все изменения публикуются в `crm.events`, где доступны события `deal.payment.updated`, `deal.payment.income.*` и `deal.payment.expense.*` для уведомлений и отчётности.【F:docs/architecture.md†L11-L66】【F:docs/tech-stack.md†L172-L204】
...
1. Установите зависимости:
   ...
   poetry install
   ```
2. Синхронизируйте переменные окружения: `../../scripts/sync-env.sh backend/crm` создаст или обновит `.env`. После копирования проверьте и заполните блок CRM (`CRM_DATABASE_URL`, `CRM_REDIS_URL`, `CRM_RABBITMQ_URL`, `CRM_EVENTS_EXCHANGE`, `CRM_CELERY_*`) и замените чувствительные данные на локальные значения.
...
```
Тесты поднимают временные контейнеры PostgreSQL/RabbitMQ/Redis, применяют миграции и проверяют REST API, включая CRUD по платежам и публикацию событий `deal.payment.*`. Для контейнера RabbitMQ используется `aio-pika`, она включена в dev-зависимости (`poetry install --with dev`).
