...
- Необходима переменная `TASKS_DATABASE_URL` (по умолчанию указывает на пользователя `crm` и `search_path=tasks` в [`env.example`](../../env.example)); остальные параметры очередей и событий берутся из блока `CRM_TASKS_*` внутри CRM.
