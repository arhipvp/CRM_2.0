...
class LoginDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        tk.Label(self, text="Username:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        self.username_var = tk.StringVar()
        tk.Entry(self, textvariable=self.username_var, width=30).grid(row=0, column=1, padx=10, pady=5)

        tk.Label(self, text="Password:").grid(row=1, column=0, padx=10, pady=5, sticky="w")
        ...
        tk.Button(button_frame, text="Login", command=self.on_login).pack(side="left", padx=5)
