...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'crm')
    ...
    EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE documents IN SCHEMA documents GRANT SELECT, REFERENCES ON TABLES TO crm';
    EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE documents IN SCHEMA documents GRANT USAGE ON SEQUENCES TO crm';
    EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE documents IN SCHEMA documents GRANT EXECUTE ON FUNCTIONS TO crm';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'crm')
    ...
    EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE auth IN SCHEMA auth GRANT SELECT, REFERENCES ON TABLES TO crm';
    EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE auth IN SCHEMA auth GRANT USAGE ON SEQUENCES TO crm';
    EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE auth IN SCHEMA auth GRANT EXECUTE ON FUNCTIONS TO crm';
  END IF;
END$$;
SQL
}
