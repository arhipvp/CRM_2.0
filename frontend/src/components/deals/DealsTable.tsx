...
export function DealsTable() {
  ...
  const recentlyUpdatedDealIds = useMemo(() => new Set(Object.keys(dealUpdates)), [dealUpdates]);
  ...
  const buildRowState = (dealId: string) => {
    ...
    return { rowClassName, handleActivate, handleKeyDown };
  };
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="flex-1 space-y-4">
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
            <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-600 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-200">
              Загружаем сделки…
            </div>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
          ...
          {hasDeals ? (
            <div className="overflow-x-auto">
              <table
                className="w-full min-w-[960px] divide-y divide-slate-100 text-sm dark:divide-slate-800"
              ...
              >
                <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500 dark:bg-slate-900/60 dark:text-slate-300">
                  <tr>
                    <th scope="col" className="px-4 py-3 text-left">Сделка</th>
                    <th scope="col" className="px-4 py-3 text-left">Клиент</th>
                    <th scope="col" className="px-4 py-3 text-left">Стадия</th>
                    <th scope="col" className="px-4 py-3 text-right">Вероятность</th>
                    <th scope="col" className="px-4 py-3 text-right">Сумма</th>
                    <th scope="col" className="px-4 py-3 text-left">Следующий просмотр</th>
                    <th scope="col" className="px-4 py-3 text-left">Ожидаемое закрытие</th>
                    <th scope="col" className="px-4 py-3 text-left">Ответственный</th>
                    <th scope="col" className="px-4 py-3 text-left">Обновлено</th>
                    <th scope="col" className="px-4 py-3 text-right">Действия</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {dealsForRender.map((deal) => {
                    const { rowClassName, handleActivate, handleKeyDown } = buildRowState(deal.id);
                    const nextReviewTone = getNextReviewTone(deal.nextReviewAt);
                    const expectedCloseDate = deal.expectedCloseDate ? new Date(deal.expectedCloseDate) : undefined;
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center justify-between text-xs font-medium">
                            <span className={classNames("flex items-center gap-2", nextReviewTone.text)}>
                            ...
                            <span className="text-slate-400 dark:text-slate-500">{formatDate(deal.nextReviewAt)}</span>
                          </div>
                        </td>
                        <td className="px-4 py-4">
