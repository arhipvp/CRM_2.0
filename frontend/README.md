...
Веб-клиент CRM построен на Next.js (App Router) с TypeScript, React Query и Zustand. Интерфейс поддерживает SSR, живые обновления через SSE и маршруты для списка сделок, карточек клиентов/сделок, задач, платежей и административной панели `/admin`.
...
- `src/stores` — Zustand-хранилища (фильтры списка сделок, административные фильтры, уведомления).
- `src/components` — модульные UI-блоки (таблицы сделок, карточки, административные модули). Для ключевых компонентов есть Vitest-тесты и Storybook stories.
